...
class RelocateEnvV0(mujoco_env.MujocoEnv, utils.EzPickle, ObsVecDict):
    ...
    def get_obs_old(self):
        ...
        return np.concatenate([qp[:-6], palm_pos-obj_pos, palm_pos-target_pos, obj_pos-target_pos, obj_pos, palm_pos, target_pos])
    ...
    def reset_model(self):
        ...
        self.model.body_pos[self.obj_bid,0] = 0 #self.np_random.uniform(low=-0.15, high=0.15)
        self.model.body_pos[self.obj_bid,1] = 0 #self.np_random.uniform(low=-0.15, high=0.3)
        ...
        self.sim.forward()
        obs = self.get_obs()
        target_pos = self.obs_dict['obj_pos'] - self.obs_dict['obj_tar_err']
    ...
    def mj_viewer_setup(self):
        self.viewer = MjRenderContextOffscreen(self.sim, -1) # MjViewer(self.sim)
