...
class GymEnv(object):
    ...
    def KoopmanVisualize(self, seed=None, state_dict=None):
        try:
            ...
            #return self.env.KoopmanVisualize(seed=seed, state_dict=state_dict)
        except:
            if seed is not None:
                self.set_seed(seed)
    ...
    def step(self, action):
        ...
        action = action.clip(np.array([-1]*len(action)), np.array([1]*len(action)))
    ...
    def generate_unseen_data_reorientate(self, number_sample):
        samples = []
        for ep in range(number_sample):
            o = self.reset(seed = ep)
            desired_orien = self.env.obs_dict['obj_des_rot']
            episode_data = {}
            ...
            objorient = o[33:36]
            episode_data['objorient'] = objorient #ori_transform(objorient, episode_data['desired_ori']) 
    ...
    def generate_unseen_data_hammer(self, number_sample):
        ...
        for ep in range(number_sample):
            ...
            episode_data['objpos'] = o[42:45] #objpos # current tool position
            episode_data['toolpos'] = o[49:52]
    ...
    def generate_unseen_data_door(self, number_sample):
        samples = []
        for ep in range(number_sample):
            o = self.reset(seed = ep) # o, desired_pos
            #pdb.set_trace()
