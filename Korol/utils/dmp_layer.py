...
def integrate(data, w, y0, dy0, goal, tau, rbf='gaussian', az=False, alpha_z=None):
    ...
    # data[2] -> simu horizon 
    if w.is_cuda:
        Y = torch.FloatTensor(w.shape[0], int(data[2].item())).fill_(0).to(w.device)
        dY = torch.FloatTensor(w.shape[0], int(data[2].item())).fill_(0).to(w.device)
        ddY = torch.FloatTensor(w.shape[0], int(data[2].item())).fill_(0).to(w.device)
    else:
    ...
    Y[:, 0] = y
    #print(f"dy0 {dy0.shape}, w {w.shape}, Y[:, 0] {Y[:, 0].shape}")
    dY[:, 0] = dy0
    ddY[:, 0] = z
    N = int(data[1].item())
    dt = data[3].item()
