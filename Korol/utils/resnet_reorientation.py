...
class ClassificationNetwork18(nn.Module):
    def __init__(self, feat_dim = 8):
        ...
        self.harmonic_embedding_dir = HarmonicEmbedding(3, 2)
        self.dir_feature = 3 * 2 * 2 + 3
        self.feature_branch = nn.Sequential(
            nn.Linear(512+self.dir_feature, 128),
            nn.ReLU(),
            nn.Linear(128, feat_dim)
        )

    def forward(self, x, desired_ori, return_embedding=False):
        feature = self.encoder(x) 
        feature_ori = self.harmonic_embedding_dir(desired_ori)
        feature_output = self.feature_branch(torch.cat((feature, feature_ori), dim=1))
