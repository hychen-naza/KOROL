...
from utils.Diffusion_evaluation import Diffusion_policy_control_door
...
dataset = DiffusionDataset(
    demo_path="./Door/Data/Testing.pickle",
    img_path='./Door/Data',
...
# device transfer
device = torch.device('cuda:0') #'cuda' # error on nvidia version
_ = nets.to(device)
...
e = GymEnv("door-v0")
e.reset()
Testing_data = e.generate_unseen_data_door(133) 
...
for epoch_idx in range(num_epochs):
    ...
    if (epoch_idx >= 100 and epoch_idx % 25 == 0):
        with torch.no_grad():
            ...
            Diffusion_policy_control_door("door-v0", Controller, ema_nets, Testing_data, dataset.stats, device, obs_horizon, pred_horizon, action_horizon, num_diffusion_iters, noise_scheduler, use_gt = False)
            # vision_encoder.eval()
            # generate_feature(vision_encoder, './Door/feature_door_demo_full.pickle', "./Door/Data", device=device, epoch=epoch_idx)
