...
def main():
    ...
    T = 100 #int(len(Training_data[0])/k)  # simulation steps
    NN_Input_size = num_obj + num_hand 
    N = 30
    ...
    for k in tqdm(range(num_demo)):
        for t in range(len(Training_data[k])):
            ...
            obj_OriState = np.append(Training_data[k][t]['objorient'], Training_data[k][t]['desired_ori'])
    ...
    for iter_num in range(1001):
        ...
        batch_loss = 0
        if (iter_num % 100 == 0 and iter_num >= 200):
            ...
            success_rate = NDP_policy_control_reorientation("pen-v0", Controller, NDP_policy, Testing_data, False, num_hand, num_obj, "Drafted", None, None)
