...
from utils.NDP_evaluation import NDP_policy_control_reorientation
#from utils.resnet_reorientation import ClassificationNetwork18
...
class NDPDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        desired_ori = path[current_idx]['desired_ori']
        count = path[current_idx]['count']
        img_path = os.path.join(self.img_path, "rgbd_"+str(count)+".npy") 
        rgbd = np.load(img_path)
        rgbd = np.transpose(rgbd, (2, 0, 1))

        return rgbd, robot_current, robot_next, obj_current, obj_next, desired_ori
...
def main():
    ...
    demo_data_path = pickle.load(open("./Reorientation/Data/Pen_task.pickle", 'rb'))
    img_path = "./Reorientation/Data"
    ...
    ndp_policy_path = './Reorientation/Results/NDP/NDP_agent.pt'
    ...
    '''
    Training_data = reorientation_demo_playback('pen-v0', demo_data_path, feature_data_path, num_demo)
    ndp_batch_num = 8
    ...
    Testing_data = e.generate_unseen_data_reorientate(133) 
    ...
    #policy.load_state_dict(torch.load(ndp_policy_path))
    ...
    for iter_num in range(1001):
        ...
        for batch_num, (rgbds, robot_currents, robot_nexts, obj_currents, obj_nexts, desired_oris) in enumerate(ndp_train_dataloader):
            robot_currents = robot_currents.float().to(device)
            desired_oris = desired_oris.float().to(device)
            rgbds = rgbds.float().to(device)
            obj_features = resnet_model(rgbds, desired_oris)
        ...
        print(f"iter_num {iter_num}, batch_loss {batch_loss}")
        batch_loss = 0
        if (iter_num % 50 == 0 and iter_num >= 50):
            ...
            success_rate = NDP_policy_control_reorientation("pen-v0", Controller, NDP_policy, Testing_data, False, num_hand, num_obj, "Drafted", resnet_model, device)
