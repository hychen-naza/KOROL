...
class BCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        obj_next = np.append(path[current_idx+1]['objpos'],  path[current_idx+1]['nail_goal'])
...
def main():
    ...
    for t in range(1001):
        ...
        for batch_num, (robot_currents, obj_currents, robot_nexts, obj_nexts) in enumerate(bc_train_dataloader):
            ...
            loss = loss_criterion(bc_output, bc_label)
            total_loss += loss.item()
            loss.backward()
            optimizer.step()
        print(f"This is {t} iteration and total_loss {total_loss}, the learning rate is {lr}")
        if (t % 100 == 0 and t >= 300):
            print("BC training ends!\n")
            BC_agent.eval()     
            BC_agent = BC_agent.to('cpu')
            with torch.no_grad():
                BC_policy_control_tool("hammer-v0", Controller, BC_agent, Testing_data, num_hand, device, None)
            BC_agent.train()     
            BC_agent = BC_agent.to(device)
