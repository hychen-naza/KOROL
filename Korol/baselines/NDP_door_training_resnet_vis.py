...
from utils.NDP_evaluation import NDP_policy_control_door
...
def main():
    ...
    demo_path = "./Door/Data/door_full_objpos_feature.pickle"
    ...
    ndp_policy_path = './Door/Results/NDP/NDP_agent.pt'
    ...
    for iter_num in range(301):
        ...
        if (iter_num % 100 == 0 and iter_num >= 300):
            ...
            NDP_policy.eval()
            resnet_model.eval()
            # success_rate = NDP_policy_control_door("door-v0", Controller, NDP_policy, Testing_data, False, num_hand, num_obj, "Drafted", resnet_model, device)
            ...
            # if (epoch >= 50 and batch_num in [0,1,2,3]): #and batch_num in [0,1,2,3]
            #     if (batch_num % 10 == 0):
            ...
            #         activation_map, activation = model.activation_vis(rgbds[i:i+1])
            ...
            #         vis_imgs[0].save(f'./tool_vis_gif/vis_{epoch}_{batch_num}.gif', save_all=True,optimize=False, append_images=vis_imgs[1:], loop=0)
            #         buf.close()
            #         plt.close(fig)
            #         # buf.close()
            #         # plt.close(fig)
