...
def main():
    ...
    for k in tqdm(range(num_demo)):
        for t in range(len(Training_data[k])):
            ...
            obj_OriState = np.append(Training_data[k][t]['objpos'], Training_data[k][t]['handle_init'])
    ...
    NDP_policy.eval()
    NDP_policy_control_door("door-v0", Controller, NDP_policy, Testing_data, False, num_hand, num_obj, "Drafted", None, None)
