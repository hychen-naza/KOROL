...
from utils.resnet_relocation import ClassificationNetwork18
...
def main():
    ...
    for k in tqdm(range(num_demo)):
        for t in range(len(Training_data[k])):
            ...
            obj_OriState = np.append(Training_data[k][t]['objpos'], Training_data[k][t]['desired_pos'])
    ...
    for iter_num in range(1001):
        ...
        batch_loss = 0
        if (iter_num % 100 == 0 and iter_num >= 500):
            ...
            success_rate = NDP_policy_control_relocate("relocate-v0", Controller, NDP_policy, Testing_data, False, num_hand, num_obj, "Drafted", None, None)
