...
from utils.BC_evaluation import BC_policy_control_reorientation # CHANGE
...
from utils.demo_loader import * # CHANGE
...
class BCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        obj_current = np.append(path[current_idx]['objorient'], path[current_idx]['desired_ori'])
        robot_next = path[current_idx+1]['handpos']     
        obj_next = np.append(path[current_idx+1]['objorient'], path[current_idx+1]['desired_ori'])
...
def main():
    ...
    env_name = 'pen-v0'
    Training_data = reorientation_demo_playback(env_name, demo_data_path, feature_data_path, num_demo)

    e = GymEnv(env_name)
    e.reset()
    Testing_data = e.generate_unseen_data_reorientate(1000) # CHANGE
    ...
    for t in range(2001):
        ...
        if (t % 200 == 0 and t >= 400):
            print("BC training ends!\n")
            BC_agent.eval()     
            BC_agent = BC_agent.to('cpu')
            with torch.no_grad():
                BC_policy_control_reorientation(env_name, Controller, BC_agent, Testing_data, num_hand, device, None)
            BC_agent.train()     
            BC_agent = BC_agent.to(device)
